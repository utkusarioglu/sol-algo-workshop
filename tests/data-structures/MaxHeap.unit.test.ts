...
import { type MaxHeap as Contract } from "_typechain/MaxHeap";
import {
  asEvmObject,
  beforeEachFacade,
  expect,
...
const CONTRACT_NAME = "MaxHeap";

function BigIntMax(...bigInts: bigint[]) {
  return BigIntSort(bigInts).at(-1);
}
            ...
            ARRAY_MODIFIERS.forEach((modifier) => {
              ...
              it(
                ...
                async () => {
                  ...
                  const response = await instance.getHead();
                  const expected = BigIntMax(...modifiedParamList);
              ...
              it(
                ...
                async () => {
                  const modifiedParamList = modifier(paramList);
                  const sortedParamList = BigIntSort(paramList).reverse();
